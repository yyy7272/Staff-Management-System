...
export function MainContent({ activeMenu, setActiveMenu, autoOpenEmployeeCreate, autoOpenDepartmentCreate, onQuickAddEmployee, onQuickAddDepartment }: MainContentProps) {
  ...
  const defaultToHome = !['home', 'organization', 'users', 'groups', 'files', 'payroll', 'permissions', 'approval', 'settings'].includes(activeMenu);
  ...
  return (
    <>
      ...
      <div style={{ display: isActive('users') ? 'block' : 'none' }}>
        <UsersPage autoOpenCreate={autoOpenEmployeeCreate} />
      </div>
