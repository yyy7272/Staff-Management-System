...
export function PayrollForm({ payroll, onSubmit, onCancel, isSubmitting = false }: PayrollFormProps) {
  const { users, isLoading: employeesLoading, loadingState: employeeLoadingState } = useUsersPage();
  ...
  // Auto-populate User salary when User is selected
  useEffect(() => {
    if (formData.employeeId && !payroll) {
      const selectedEmployee = users.find(emp => emp.id === formData.employeeId);
      if (selectedEmployee && selectedEmployee.salary) {
        setFormData(prev => ({ ...prev, baseSalary: selectedEmployee.salary }));
      }
    }
  }, [formData.employeeId, users, payroll]);
  ...
  if (employeeLoadingState === 'error' || (users.length === 0 && !employeesLoading)) {
    return (
      <div className="space-y-4">
        <Alert variant="destructive">
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            Failed to load User data. You may not have permission to access User information, 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Select 
            ...
            disabled={!!payroll} // Don't allow changing User for existing payroll
          >
            ...
            <SelectContent>
              {users.map((User) => (
                <SelectItem key={User.id} value={User.id}>
                  {User.name} - {typeof User.department === 'string' ? User.department : User.department?.name}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
